...
export function Character01() {
  ...
  const handlePress = async () => {
    try {
      ...
      //vehicles
      const vehiclesResponses = await Promise.all(json.vehicles.map((vehicleUrl) => fetch(vehicleUrl)));
      const vehiclesJson = await Promise.all(vehiclesResponses.map((vehiclesResponses) => vehiclesResponses.json()));
      setVehicle([vehiclesJson]);


      //starships
      const starshipsResponses = await Promise.all(json.starships.map((starshipUrl) => fetch(starshipUrl)));
      const starshipsJson = await Promise.all(starshipsResponses.map((starshipsResponses) => starshipsResponses.json()));
      setStarship([starshipsJson]);
        ...
        <View>
          <ScrollView>
            ...
            {vehicles.map((vehicle) => (
              <>
                <Text style={styles.text}>{vehicle[0].name}</Text>
                <Text style={styles.text}>{vehicle[1].name}</Text>
              </>
            ))}
            ...
            {starships.map((starship) => (
              <>
                <Text style={styles.text}>{starship[0].name}</Text>
                <Text style={styles.text}>{starship[1].name}</Text>
              </>
            ))}
